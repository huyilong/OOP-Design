...
//Queue<Integer> myNumbers = new LinkedList<Integer>();
    ...
    //The only thing we need to take care of is the push method
    public void push(int x) {
        if(empty()){
            ...
            //step1: getting all of the elements in stack to temp
            while(!stack.isEmpty()){
                temp.offer(stack.poll());
            }
            //step2: add into the stack queue
            stack.offer(x);
            
            //step3: add back all the elements in temp to stakc
            while(!temp.isEmpty()){
